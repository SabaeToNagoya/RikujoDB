      ...
      <tbody>
        {results.map((r) => (
          <tr key={r.id} style={{ cursor: "pointer" }} onClick={() => onEdit(r)}>
            ...
            <td>
              <RankBadge rank={r.ranking} />
            </td>
            <td
              style={{
                fontWeight: r.totalTime ? 500 : undefined,
                color: r.totalTime ? undefined : "var(--color-text-tertiary)",
              }}
            >
              {r.totalTime || "—"}
            </td>
...
export default function TeamDetailPage() {
  ...
  const saveResult = async () => {
    ...
    const body = {
      teamId: id,
      competitionName: resultForm.competitionName,
      type: resultForm.type,
      year: parseInt(resultForm.year),
      ranking: parseInt(resultForm.ranking),
      totalTime: resultForm.totalTime || null,
      notes: resultForm.notes || null,
    };
    if (editingResult) {
      await fetch(`/api/team-results/${editingResult.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
    } else {
      await fetch("/api/team-results", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", gap: "12px", marginBottom: "0.9rem" }}>
        ...
        <button className="btn btn-primary" onClick={openAddResult}>
          + 成績を追加
        </button>
      </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px", marginBottom: "0.9rem" }}>
          <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "0.6rem 0.8rem" }}>
            ...
            <div style={{ fontSize: "18px", fontWeight: 500 }}>
              {team.results.filter((r) => r.ranking === 1).length}回
            </div>
          </div>
          <div style={{ background: "var(--color-background-secondary)", borderRadius: "var(--border-radius-md)", padding: "0.6rem 0.8rem" }}>
      ...
      <div className="card">
        <div className="tab-bar">
          <button
            className={`tab-item ${activeTab === "athletes" ? "active" : ""}`}
          ...
          <button
            ...
            onClick={() => setActiveTab("ekiden")}
          >
            駅伝成績
          </button>
          <button
            className={`tab-item ${activeTab === "marathon" ? "active" : ""}`}
        ...
        {activeTab === "ekiden" && (
          <>
            {ekidenResults.length > 0 && (
              <RankHistChart results={team.results} compType="駅伝" />
            )}
            {ekidenResults.length === 0 ? (
              <div style={{ fontSize: "11px", color: "var(--color-text-tertiary)", padding: "0.5rem 0" }}>
                駅伝成績がありません
      ...
      <Modal
        ...
        title={editingResult ? "成績を編集" : "成績を追加"}
      >
        <div style={{ display: "grid", gap: "0.65rem" }}>
          ...
          <div className="two-col" style={{ gap: "8px" }}>
            <div>
              ...
              <select
                ...
                onChange={(e) => rf("type", e.target.value)}
              >
                {RESULT_TYPES.map((t) => (
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => rf("year", e.target.value)}
                placeholder="2025"
              />
            </div>
          </div>
          <div className="two-col" style={{ gap: "8px" }}>
            ...
            <div>
              ...
              <input
                className="form-input"
                value={resultForm.totalTime}
                onChange={(e) => rf("totalTime", e.target.value)}
                placeholder="10:41:19"
              />
            </div>
          </div>
          <div>
          ...
          <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
            <button className="btn" onClick={() => setResultModal(false)}>
              キャンセル
            </button>
            <button
              className="btn btn-primary"
              onClick={saveResult}
              disabled={saving || !resultForm.competitionName || !resultForm.ranking}
            >
              {saving ? "保存中..." : "保存"}
            </button>
          </div>
